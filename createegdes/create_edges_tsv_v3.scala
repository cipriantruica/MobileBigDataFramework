...
/*
    ...
    It save the data to parquet files.
    It reads the parquet files.
    It computes the edges.
    It saves the edges as parquet
*/

object CreateEdgesSparkSQLTSV_v3 {
  def main(args: Array[String]): Unit = {
        /// input directory with the tsv files
        val inputDirectory = "hdfs://hadoop-master:8020/user/ciprian/input/MI2MI/MItoMI-2013-11*" 
        // val inputDirectory = args(0)

        // input/output file with parquet files
        val inputOutputFile = "hdfs://hadoop-master:8020/user/ciprian/output/MI2MI/parquet" 
        ...
        pw.println("Create_Edges_SparkSQL_TSV_v3!")
        pw.println("Start time: " + Calendar.getInstance().getTime())

        val t0 = System.nanoTime()

        // Spark session
        val spark = SparkSession.builder.appName("Create_Edges_SparkSQL_TSV_v3").getOrCreate;
        ...
        df.select(from_unixtime(df("Timestamp")/1000, "yyyy-MM-dd hh:mm:ss").as("Timestamp"), df("SquareID1"), df("SquareID2"), df("DIS"))
            .write
            .option("codec", "snappy") 
            .parquet(inputOutputFile)

        // this returns a data frame
        val dfParquet = spark.read.parquet(inputOutputFile)
