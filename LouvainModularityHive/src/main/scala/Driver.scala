...
object Driver {
  ...
  def main(args: Array[String]): Unit ={
    ...
    val exists = hc.sql("select count(*) NrRecords from " + config.hiveOutputTable + " where MilanoDate = '" + config.dateInput + "' and alphaThreshold = " + config.alphaThreshold + " and edgeCostFactor =" + config.edgeCostFactor)
    exists.show()
